...
var SileroVADAdapter = class {
  constructor(modelUrl) {
    ...
    this.sr = new ort__namespace.Tensor("int64", new BigInt64Array([16000n]));
  }
  async load() {
    if (this.loaded) return;
    try {
      this.session = await ort__namespace.InferenceSession.create(this.modelUrl);
      this.reset();
      this.loaded = true;
    } catch (e) {
  ...
  async process(audioFrame) {
    ...
    const input = new ort__namespace.Tensor("float32", audioFrame, [1, audioFrame.length]);
    const feeds = {
      input,
      sr: this.sr,
      h: this.h,
  ...
  reset() {
    ...
    this.h = new ort__namespace.Tensor("float32", zeros, [2, 1, 64]);
...
var useLiveAudio = ({ transport, sampleRate = 24e3 }) => {
  ...
  const [frequencyData, setFrequencyData] = react.useState(new Float32Array(0));
  const recorderRef = react.useRef(null);
...
exports.useAudioSocket = useAudioSocket;
